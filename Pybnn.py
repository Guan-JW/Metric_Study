...
parser.add_argument('--data', type=str, default="Toy", help='Dataset number')
...
parser.add_argument('--ult_objs', type=str, nargs='+', default=['test_loss'], help='List of strings (default: ["test_accuracy", "test_losses"])')
parser.add_argument('--max_iters', type=int, nargs='+', default=[500, 2500, 4500, 6500], help='List of integers (default: [20, 50, 81, 120, 150])')
...
test_acc_file = os.path.join(dir_path, "test_loss.csv")
...
if 'seed' in args.task:
	...
	min_config = total_sample_num

	for s in data_seed:
		dir_path = os.path.join(dir_path, f"{s}")
		val_loss_file = os.path.join(dir_path, "val_loss.csv")
		train_loss_file = os.path.join(dir_path, "train_loss.csv")
		test_acc_file = os.path.join(dir_path, "test_loss.csv")

		df_val_loss_seed = pd.read_csv(val_loss_file, header=None)
		df_train_loss_seed = pd.read_csv(train_loss_file, header=None)
		df_test_acc_seed = pd.read_csv(test_acc_file, header=None)
		min_config = min(df_val_loss_seed.shape[0], df_train_loss_seed.shape[0], df_test_acc_seed.shape[0])
		
		df_val_loss = df_val_loss.iloc[:min_config, :] + df_val_loss_seed.iloc[:min_config, 1:]
		df_train_loss = df_train_loss.iloc[:min_config, :] + df_train_loss_seed.iloc[:min_config, 1:]
		df_test_acc = df_test_acc.iloc[:min_config, :] + df_test_acc_seed.iloc[:min_config, 1:]
	
	df_val_loss /= len(data_seed)
	df_train_loss /= len(data_seed)
	df_test_acc /= len(data_seed)

	total_sample_num = min_config
...
def try_params(n_iteration, config, criteria='valid_accuracy', last_epoch=0):
	...
	fidelity = {'epoch': round(n_iteration) + 1}	# NN fidelity range [1, 200]
	print(f"{fidelity=}")
	...
	rtn_dic = {'time': 0,
			   'test_losses': df_test_acc.iloc[config, 0]}
