...
NB201_Cifar100 = Cifar100NasBench201Benchmark(rng=1)
config_space = NB201_Cifar100.get_configuration_space(seed=1)
...
def try_params(n_iteration, config, criteria='valid_accuracy'):
	...
	result_dic = NB201_Cifar100.objective_function(configuration=config, fidelity=fidelity, data_seed=data_seed)
	final_rst_dic = NB201_Cifar100.objective_function(configuration=config, fidelity=final_fidelity, data_seed=data_seed)
	...
	if 'stage' in criteria:
			...
			if 'win' in criteria:
				...
				if round(n_iteration) < window_size:
					for i in range(1, round(n_iteration) + 2):
						...
						hist_dic = NB201_Cifar100.objective_function(configuration=config, fidelity=hist_fidelity, data_seed=data_seed)
		...
		last_dic = NB201_Cifar100.objective_function(configuration=config, fidelity=last_fidelity, data_seed=data_seed)
		if 'valid' in criteria:
			rcr = 0 - (result_dic['info']['valid_losses'] - last_dic['info']['valid_losses'])   # minimize
			rtn_dic['valid_losses'] = result_dic['info']['valid_losses']
		elif 'train' in criteria:
			rcr = 0 - (result_dic['info']['train_losses'] - last_dic['info']['train_losses'])
